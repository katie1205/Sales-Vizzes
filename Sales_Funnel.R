...
# dataset <- data.frame(Weighted Quoted Opportunity Value, Sort2, Phases, Today)
# dataset <- unique(dataset)

# Paste or type your script code here:

#Load libraries
library(graphics) #for graphing polygons
library(RColorBrewer) #for color palettes
library(dplyr) #for selecting, sorting, and aggregating
library(gridExtra) #for displaying a table 

#Fix spaces in field names 
names(dataset) <- gsub(" ","_",names(dataset))  #substitute spaces with underscores

#Replace blanks with na
apply(dataset, 2, function(x) gsub("^$|^ $", NA, x))
summary<-dataset%>%select(Weighted_Quoted_Opportunity_Value, Sort2, Phases)%>%arrange(Sort2)  #sorting the phases in chronological order
